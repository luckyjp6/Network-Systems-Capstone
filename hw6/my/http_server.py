        ...
        while self.alive:
            try:
                ...
                if self.version == "HTTP/1.1":
                    # parse request
                    method, path, params, version, headers, body = self.__parse_reqeust(request.decode())
                    if "Connection" in headers and "Upgrade" in headers and "HTTP2-Settings" in headers:
                ...
                if version == "HTTP/1.0" and not ('Connection' in headers and headers['Connection'] == 'keep-alive'):
                    self.alive = False
                    self.client.close()
                    break
            except:
