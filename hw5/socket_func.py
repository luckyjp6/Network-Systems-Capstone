...
def send_loop(s:socket.socket):
    ...
    while True:
        ...
        if wait_to_send:            
            ...
            send_amount_max = min(send_amount, len(wait_to_send)) # packet/send
        ...
        if num_send > 0: 
            ...
            elif send_rate < 100: send_amount += 3
            num_ack = 0
            num_send = 0
        send_lock.release()
...
def recv_loop(s:socket.socket):
    ...
    while True:
        ...
        if (packet == None) or (num_recv >= send_amount*send_window): 
            ...
            global send_lock, wait_to_send, num_ack, pn_to_send
...
def send_ack(s:socket.socket):
    global recv_pn, send_lock # , reply_queue, reply_lock
    # send ack
    ack_packet = fr.ACK_frame()
    total_num, acks = recv_pn.get_needed()
    if total_num == 0: return
    # print(total_num, acks)
    ack_packet.set(total_num, acks)
